...
void matches2points_nndr(const std::vector<FeaKeyPoint>& train,
                         const std::vector<FeaKeyPoint>& query,
...
void draw_inliers_ellipse(Mat& img, std::vector<FeaKeyPoint>& kpts)
{
...
int read_file_kpts_desc(const char* filename, std::vector<FeaKeyPoint>& kpts, Mat& desc)
{
	ifstream read_file;
...
void divide_inliers_kpts(std::vector<Point2f> inliers, std::vector<FeaKeyPoint>& kpts1_inliers, std::vector<FeaKeyPoint>& kpts2_inliers)
{
...
void compute_additional_message_kpts(std::vector<FeaKeyPoint>& kpts, std::vector<FeaKeyPoint>& kpts_inliers)
{
...
void compute_MatchImgRMSE(std::vector<Point2f>& inliers, Mat& Homography, double& rmse)
{
	...
	FeaKeyPoint data;
	vector<FeaKeyPoint>::iterator  it;
	it = kpts1.begin();
	...
	vector<FeaKeyPoint>::iterator it1,it2;
	it1 = xkpts1.begin();
	it2 = kpts2.begin();
	double RMSE = 0;
